...
/**
 * Modal/offcanvas manager for shared-shell loaders and unique dynamic overlays.
 *
 * What it supports:
 * - Standard Bootstrap modal and offcanvas creation with stack handling
 * - HTML/API content loading with retry, inline error state, and CSRF sync
 * - Shared container loading through loadFileContent/loadFormContent
 * - Dynamic overlay loading through showFileContent/showFormContent
 * - Form hydration plus optional setup hooks like getPassData(baseUrl, data)
 * - Optional skeleton loading for forms/content when a loading placeholder helps the UX
 *
 * Common shared-shell examples:
 * const manager = modalManager();
 *
 * // Shared offcanvas form.
 * manager.loadFormContent({
 ...
 * });
 *
 * Common standalone overlay examples:
 *
 * // Standalone modal.
 * manager.showFileContent({
 ...
 * });
 *
 * // Standalone offcanvas form.
 * manager.showFormContent({
 *     fileName: 'views/directory/_userForm.php',
 ...
 *
 * Skeleton behavior:
 * - showSkeleton: true  -> render the configured skeleton before content loads
 * - showSkeleton: false -> keep the container empty while loading
 * - skeletonVariant     -> choose a specific skeleton layout when skeletons are enabled
 */
class ModalManager {
    constructor() {
        this.activeModals = new Map();
        this.activeOffcanvas = new Map();
        this.overlayStackSequence = 0;
    ...
    createOffcanvasInstance(element, options = {}) {
        if (this.canUseBootstrapOffcanvas() && element.classList.contains('offcanvas')) {
            if (typeof window.bootstrap.Offcanvas.getOrCreateInstance === 'function') {
                return window.bootstrap.Offcanvas.getOrCreateInstance(element, options);
            }
    ...
    blurFocusedDescendant(containerElement) {
    ...
    getOverlayStackOrder(element, fallbackIndex = 0) {
        if (!(element instanceof HTMLElement)) {
            return fallbackIndex;
        }

        const stackOrder = parseInt(element.getAttribute('data-overlay-stack-order') || '', 10);
        return Number.isNaN(stackOrder) ? fallbackIndex : stackOrder;
    }

    markOverlayAsTopmost(element) {
        if (!(element instanceof HTMLElement)) {
            return;
        }

        this.overlayStackSequence += 1;
        element.setAttribute('data-overlay-stack-order', String(this.overlayStackSequence));
    }
    ...
    reindexOverlayStack() {
        ...
        const overlays = this.getVisibleOverlayElements()
            .map((element, index) => ({
                element,
                order: this.getOverlayStackOrder(element, index),
            }))
    ...
    initializeOverlayStackManager() {
        ...
        document.addEventListener('show.bs.modal', (event) => {
            this.markOverlayAsTopmost(event.target);
    ...
    /**
     * Create a new modal instance with a unique id.
     * Use this when you need a standalone modal rather than the shared content shell.
     */
    showModal(options = {}) {
        try {
    ...
    /**
     * Create a standalone modal and hydrate it from an HTTP endpoint.
     */
    ...
    getModalContentUrl() {
        return $('meta[name="route.modal.content"]').attr('content') || 'modal/content';
    }

    normalizeSharedFileOptions(...args) {
        const defaults = {
            fileName: '',
            idToLoad: 'generalContent',
            sizeModal: 'lg',
        ...
        const [fileName, idToLoad, sizeModal = 'lg', title = 'Default Title', dataArray = null, typeModal = 'modal', retryOnCsrfMismatch = true] = args;
        return {
            ...defaults,
            fileName,
            idToLoad,
    ...
    normalizeSharedFormOptions(...args) {
        const defaults = {
            fileName: '',
            idToLoad: 'generalContent',
            sizeModal: 'lg',
        ...
        const [fileName, idToLoad, sizeModal = 'lg', urlFunc = null, title = 'Default Title', dataArray = null, typeModal = 'modal', retryOnCsrfMismatch = true] = args;
        return {
            ...defaults,
            fileName,
            idToLoad,
    ...
    getSharedMountElement(container) {
        return document.getElementById(container.mountId || container.contentId);
    }

    applySharedLoadingState(container, config) {
        const titleElement = document.getElementById(container.titleId);
        if (titleElement) {
            titleElement.textContent = config.title;
        }

        const mountElement = this.getSharedMountElement(container);
        if (mountElement) {
            if (config.showSkeleton === false) {
                mountElement.innerHTML = '';
            } else {
                mountElement.innerHTML = this.createOverlaySkeleton({ variant: config.skeletonVariant });
            }
        }

        this.showSharedContainer(container, config.typeModal);
        return mountElement;
    }

    mountSharedResponseContent(mountElement, html) {
        if (!mountElement) {
            return;
        }

        this.mountHtmlContent(mountElement, html);
        if (typeof ensureCsrfFieldsInContainer === 'function') {
            ensureCsrfFieldsInContainer(mountElement);
        }
    }

    configureSharedForm(mountElement, container, config) {
    ...
    handleContentSetupError(error, config) {
        const fallbackMessage = config.isForm
            ? 'Content loaded but form initialization failed'
            : 'Content loaded but initialization failed';
        const message = error && error.message ? error.message : fallbackMessage;

        console.error(error || message, config.isForm ? 'ERROR loadFormContent setup' : 'ERROR loadFileContent setup');
    ...
    handleContentRequestFailure(requestError, config, mountElement, shellId) {
        const statusCode = requestError && requestError.xhr && requestError.xhr.status ? requestError.xhr.status : 500;
        const defaultMessage = config.isForm ? 'Failed to load form content' : 'Failed to load modal content';
        const message = requestError && requestError.message ? requestError.message : defaultMessage;

        if (mountElement) {
            mountElement.innerHTML = this.createContentLoadErrorHtml(message, shellId);
        }

        if (config.isForm && typeof isError === 'function' && isError(statusCode) && typeof noti === 'function') {
            noti(statusCode, message);
            return;
        }

        if (config.isForm && typeof isUnauthorized === 'function' && isUnauthorized(statusCode) && typeof noti === 'function') {
    ...
    async loadSharedOverlayContent(config) {
        const container = this.prepareSharedContainer(config.idToLoad, config.sizeModal, config.typeModal);
        const mountElement = this.applySharedLoadingState(container, config);

        const runLoad = async () => {
            const response = await this.requestHtmlContent({
                method: 'POST',
                url: this.getModalContentUrl(),
                data: {
            ...
            this.mountSharedResponseContent(mountElement, response.data);

            if (config.isForm) {
                this.configureSharedForm(mountElement, container, config);
            }

            try {
                await this.invokeContentSetupHook(config, response.data);
            } catch (setupError) {
                this.handleContentSetupError(setupError, config);
            }

            this.clearContentRetry(container.shellId);
            return response;
        };

        try {
            this.registerContentRetry(container.shellId, runLoad);
            return await runLoad();
        } catch (requestError) {
            this.handleContentRequestFailure(requestError, config, mountElement, container.shellId);
            throw requestError;
        }
    }

    ensureSharedModalContainer(size) {
    ...
    ensureSharedOffcanvasContainer(width = '400px') {
    ...
    prepareSharedContainer(idToLoad, sizeModal, typeModal) {
        if (typeModal === 'modal') {
            const container = this.ensureSharedModalContainer(sizeModal);
        ...
        const offcanvasContainer = this.ensureSharedOffcanvasContainer(sizeModal);
    ...
    showSharedContainer(container, typeModal) {
    ...
    getControllerShellId(controller) {
        if (!controller || typeof controller !== 'object') {
            return null;
        }

        return controller.modalId || controller.offcanvasId || null;
    }

    getControllerContentElement(controller) {
        if (!controller || !controller.contentId) {
            return null;
        }

        return document.getElementById(controller.contentId);
    }

    createFieldSkeleton(width = '100%') {
        return '<div class="modal-skeleton__field">'
            + '<div class="modal-skeleton__label skeleton" style="width: 34%"></div>'
            + '<div class="modal-skeleton__control skeleton" style="width: ' + width + '"></div>'
            + '</div>';
    }

    createOrderedFormSkeleton() {
        return [
            '<div class="modal-skeleton modal-skeleton--form" aria-hidden="true">',
            '    <div class="modal-skeleton__stack">',
            '        <div class="modal-skeleton__field modal-skeleton__field--full">',
            '            <div class="modal-skeleton__label skeleton" style="width: 24%"></div>',
            '            <div class="modal-skeleton__control skeleton" style="width: 100%"></div>',
            '        </div>',
            '        <div class="modal-skeleton__field modal-skeleton__field--full">',
            '            <div class="modal-skeleton__label skeleton" style="width: 28%"></div>',
            '            <div class="modal-skeleton__control skeleton" style="width: 100%"></div>',
            '        </div>',
            '        <div class="modal-skeleton__field modal-skeleton__field--full">',
            '            <div class="modal-skeleton__label skeleton" style="width: 16%"></div>',
            '            <div class="modal-skeleton__control skeleton" style="width: 100%"></div>',
            '        </div>',
            '        <div class="modal-skeleton__grid modal-skeleton__grid--paired">',
            '            <div class="modal-skeleton__field">',
            '                <div class="modal-skeleton__label skeleton" style="width: 30%"></div>',
            '                <div class="modal-skeleton__control skeleton" style="width: 100%"></div>',
            '            </div>',
            '            <div class="modal-skeleton__field">',
            '                <div class="modal-skeleton__label skeleton" style="width: 24%"></div>',
            ...
            '        <div class="modal-skeleton__grid modal-skeleton__grid--paired">',
            '            <div class="modal-skeleton__field">',
            '                <div class="modal-skeleton__label skeleton" style="width: 32%"></div>',
            '                <div class="modal-skeleton__control skeleton" style="width: 100%"></div>',
            '            </div>',
            '            <div class="modal-skeleton__field">',
            '                <div class="modal-skeleton__label skeleton" style="width: 18%"></div>',
            '                <div class="modal-skeleton__control skeleton" style="width: 100%"></div>',
            '            </div>',
            '        </div>',
            '    </div>',
            '    <div class="modal-skeleton__footer">',
            '        <div class="modal-skeleton__hint skeleton"></div>',
            '        <div class="modal-skeleton__button skeleton"></div>',
            '    </div>',
            '</div>'
        ].join('');
    }

    createOverlaySkeleton(options = {}) {
        const config = {
            variant: 'form',
            fieldCount: 6,
            showActions: true,
        ...
        if (config.variant === 'content') {
            return [
                '<div class="modal-skeleton modal-skeleton--content" aria-hidden="true">',
                '    <div class="modal-skeleton__hero skeleton"></div>',
                '    <div class="modal-skeleton__block skeleton"></div>',
                '    <div class="modal-skeleton__block skeleton" style="width: 92%"></div>',
                '    <div class="modal-skeleton__block skeleton" style="width: 78%"></div>',
                '    <div class="modal-skeleton__cards">',
                '        <div class="modal-skeleton__card skeleton"></div>',
                '        <div class="modal-skeleton__card skeleton"></div>',
                '    </div>',
                '</div>'
            ].join('');
        ...
        if (config.variant === 'ordered-form') {
            return this.createOrderedFormSkeleton();
        }

        const skeletonFields = [];
        for (let index = 0; index < config.fieldCount; index++) {
            const width = index % 2 === 0 ? '100%' : '88%';
            skeletonFields.push(this.createFieldSkeleton(width));
        }

        const actionsHtml = config.showActions
            ? '<div class="modal-skeleton__actions"><div class="modal-skeleton__button skeleton"></div><div class="modal-skeleton__button modal-skeleton__button--secondary skeleton"></div></div>'
            : '';

        return [
            '<div class="modal-skeleton modal-skeleton--form" aria-hidden="true">',
            '    <div class="modal-skeleton__row">',
            '        <div class="modal-skeleton__title skeleton"></div>',
            '        <div class="modal-skeleton__chip skeleton"></div>',
            '    </div>',
            '    <div class="modal-skeleton__grid">',
            skeletonFields.join(''),
            '    </div>',
            actionsHtml,
            '</div>'
    ...
    registerContentRetry(shellId, retryHandler) {
        if (!shellId || typeof retryHandler !== 'function') {
            return;
        }

        window.__modalContentRetry = window.__modalContentRetry || {};
        window.__modalContentRetry[shellId] = async () => {
            try {
                await retryHandler();
            } catch (error) {
                console.error('Error retrying overlay content:', error);
            }
        };
    }

    clearContentRetry(shellId) {
        if (!shellId || !window.__modalContentRetry || !window.__modalContentRetry[shellId]) {
            return;
        }

        delete window.__modalContentRetry[shellId];
    }

    createDynamicOverlay(options = {}) {
        const config = {
            overlayType: 'modal',
            title: 'Default Title',
            size: null,
        ...
        const overlayType = config.overlayType === 'offcanvas' ? 'offcanvas' : 'modal';
    const content = config.content || (config.showSkeleton === false ? '' : this.createOverlaySkeleton({ variant: config.skeletonVariant }));

        if (overlayType === 'offcanvas') {
            return this.showOffcanvas({
                title: config.title,
                width: config.size || config.width || this.defaultOffcanvasConfig.width,
                content,
                ...
                offcanvasClass: config.offcanvasClass || '',
        ...
        return this.showModal({
            title: config.title,
            size: config.size || this.defaultModalConfig.size,
            content,
            backdrop: config.staticBackdrop ? 'static' : config.backdrop,
            keyboard: config.keyboard,
            focus: config.focus,
            ...
            staticBackdrop: config.staticBackdrop,
            footerButtons: Array.isArray(config.footerButtons) ? config.footerButtons : [],
        });
    }

    async hydrateDynamicOverlay(controller, config = {}) {
        const contentElement = this.getControllerContentElement(controller);
        const shellId = this.getControllerShellId(controller);
        ...
        const runLoad = async () => {
            const response = await this.requestHtmlContent({
                method: 'POST',
                url: $('meta[name="route.modal.content"]').attr('content') || 'modal/content',
                data: requestPayload,
                retryOnCsrfMismatch: config.retryOnCsrfMismatch,
            });

            if (contentElement) {
                this.mountHtmlContent(contentElement, response.data);
                if (typeof ensureCsrfFieldsInContainer === 'function') {
                    ensureCsrfFieldsInContainer(contentElement);
                }
            }

            const formElement = contentElement ? contentElement.querySelector('form') : null;
            if (formElement instanceof HTMLFormElement) {
                const controllerShellId = this.getControllerShellId(controller);
                if (controllerShellId) {
                    formElement.setAttribute('data-modal', '#' + controllerShellId);
                }
            ...
            if (config.isForm === true && formElement instanceof HTMLFormElement) {
                formElement.reset();

                if (config.formAction !== null && config.formAction !== undefined) {
                    formElement.setAttribute('action', config.formAction);
                }
            ...
            await this.invokeContentSetupHook(config, response.data);
            this.clearContentRetry(shellId);
            return response;
        };

        try {
            this.registerContentRetry(shellId, runLoad);
            return await runLoad();
        } catch (requestError) {
            if (contentElement) {
                contentElement.innerHTML = this.createContentLoadErrorHtml(requestError.message || 'Failed to load content', shellId);
            }

            if (typeof noti === 'function') {
                const statusCode = requestError && requestError.xhr && requestError.xhr.status ? requestError.xhr.status : 500;
                noti(statusCode, requestError.message || 'Failed to load content');
            }
    ...
    /**
     * Open a standalone overlay and load an HTML partial into it.
     * Best for nested overlays where you do not want to reuse the shared shell.
     * Set showSkeleton=false when the partial should open without a loading skeleton.
     */
    async showFileContent(options = {}) {
        const config = {
            fileName: '',
            title: 'Default Title',
            dataArray: null,
        ...
        const controller = this.createDynamicOverlay(config);
        await this.hydrateDynamicOverlay(controller, config);
        return controller;
    }

    /**
     * Open a standalone overlay, load a form partial, set its action, and hydrate its values.
     * Example:
     * manager.showFormContent({
     *     fileName: 'views/directory/_userForm.php',
     *     overlayType: 'offcanvas',
    ...
    *     showSkeleton: false
    ...
    /**
     * Load an HTML partial into the shared modal/offcanvas shell.
     * Supports keyed config or positional arguments.
     * Set showSkeleton=false when you want no loading placeholder.
     */
    async loadFileContent(...args) {
        const config = this.normalizeSharedFileOptions(...args);
        return this.loadSharedOverlayContent(config);
    }

    /**
     * Load a form partial into the shared modal/offcanvas shell.
     * Supports keyed config or positional arguments.
     * Preferred setup:
     * manager.loadFormContent({
     *     fileName: 'views/directory/_userForm.php',
    ...
    *     showSkeleton: false
     * });
     */
    async loadFormContent(...args) {
        const config = this.normalizeSharedFormOptions(...args);
        return this.loadSharedOverlayContent(config);
    }
        ...
        try {
            ...
            modalElement.addEventListener('hidden.bs.modal', () => {
                try {
                    ...
                    this.clearContentRetry(controller.modalId);
                } catch (error) {
    ...
    /**
     * Create a standalone offcanvas and hydrate it from an HTTP endpoint.
     */
    ...
    addOffcanvasEventListeners(offcanvasElement, config, controller) {
        try {
            ...
            offcanvasElement.addEventListener(hideEventName, () => {
                this.blurFocusedDescendant(offcanvasElement);
            });
            
            // Auto cleanup on hide
            offcanvasElement.addEventListener(hiddenEventName, () => {
                try {
                    if (controller.offcanvas && typeof controller.offcanvas.dispose === 'function') {
                        controller.offcanvas.dispose();
                    }
                    ...
                    this.clearContentRetry(controller.offcanvasId);
                } catch (error) {
